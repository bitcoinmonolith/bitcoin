...
export const BIP_65_OPCODE_TABLE: OPCODE_TABLE = [...OPCODE_TABLE_GENESIS];

export const OP_CHECKLOCKTIMEVERIFY = 0xb1; // formerly OP_NOP2
BIP_65_OPCODE_TABLE[OP_CHECKLOCKTIMEVERIFY] = ({ stack, tx }) => {
	// Stack must have at least one item
	if (stack.length < 1) {
		throw new Error("OP_CHECKLOCKTIMEVERIFY: Stack underflow");
	}
	...
	if (nLocktime < 0) {
		throw new Error("OP_CHECKLOCKTIMEVERIFY: Negative locktime");
	}

	// 2. Transaction must not be finalized (sequence must not be max)
	if (tx.inputs.some((input) => input.sequence === 0xffffffff)) {
		throw new Error("OP_CHECKLOCKTIMEVERIFY: Transaction is finalized");
	}
	...
	if (stackLockTimeType !== txLockTimeType) {
		throw new Error("OP_CHECKLOCKTIMEVERIFY: Locktime type mismatch");
	}

	// 4. Transaction locktime must be greater or equal to stack locktime
	if (tx.locktime < nLocktime) {
		throw new Error("OP_CHECKLOCKTIMEVERIFY: Transaction locktime is less than stack locktime");
	}
};
